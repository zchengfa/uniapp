			...
			<swiper-item  v-for="(item,index) in MVData" :key="index">
				<view class="scroll-content swiper-item">
					<view class="scroll-item">
						<view   class="my-video" >
							<cover-view>
								<video style="width: 100%;" :id="'myVideo' + index" :src="item.url" :autoplay="item.autoplay" :poster="item.detail.cover ||item.detail.coverUrl" loop="true" objectFit="contain" @play="play(index)" @pause="pause(index)" @ended="videoEnded"></video>
								...
								<view class="operation">
									...
									<view class="operation-item song">
										<view class="pic-box" :class="item.playing ? 'rotate' : 'rotate paused'">
											<image :src="item.detail.cover ||item.detail.coverUrl" class="cover" mode="aspectFill"></image>
										</view>
									</view>
								</view>
								<view class="info">
									<view class="artist-box info-item">
										<view class="artist-avatar" v-if="Number(vId)">
											<image class="artist-ava" v-for="(art,artIndex) in item.detail.artists" :src="art.img1v1Url" :key="artIndex"></image>
										</view>
										<view class="artist-avatar" v-else>
											<image class="artist-ava" :src="item.detail.avatarUrl"></image>
										</view>
										<text class="artist-name" v-if="Number(vId)">{{$dealAuthor(item.detail.artists,'name')}}</text>
									...
									<view class="mv-name-desc  info-item" @tap="showDesc(index)">
										...
										<text class="top-angle angle" v-if="!item.isShowDesc && (item.detail.desc || item.detail.description)"></text>
									</view>
...
<script>
	...
	import { vInfo , vUrl , vDetail, relatedMV} from '@/common/api.js'
			...
			getVDetail(id,index){
				vDetail(id).then(res=>{
					if(res.code === 200){
						...
						//处理时间
						this.MVData[index].detail && Number(this.MVData[index].detail.publishTime) ? this.MVData[index].detail.publishTime = timeFormatting('YYYY-MM-DD',this.MVData[index].detail.publishTime) : null
